...
THISCHUNK=0
CONTAINER_BIN=""
COMPOSER_ROOT_VERSION="14.2.x-dev"
...
case ${TEST_SUITE} in
    acceptance)
        ...
        ${CONTAINER_BIN} run --rm ${CI_PARAMS} -d ${SELENIUM_GRID} --name ac-chrome-${SUFFIX} --network ${NETWORK} --network-alias chrome ${IMAGE_SELENIUM} >/dev/null
        SUITE_EXIT_CODE=$? && [[ "${SUITE_EXIT_CODE}" -ne 0 ]] && printSummary
        if [ ${CONTAINER_BIN} = "docker" ]; then
    ...
    checkGitSubmodule)
        ...
        ;;
    checkGruntClean)
        COMMAND="find 'typo3/sysext' -name '*.js' -not -path '*/theme_camino/*' -not -path '*/Fixtures/*' -exec rm '{}' + && cd Build; npm ci || exit 1; node_modules/grunt/bin/grunt build; cd ..; git add *; git status; git status | grep -q \"nothing to commit, working tree clean\""
